...
const run = (function () {
    ...
    const functions = {
        ...
        코로나: function () {
            return type0(org.jsoup.Jsoup.connect("http://ncov.mohw.go.kr/").get().select("ul.liveNum>li").toArray().map(x=>(x.select("strong.tit")+" : "+x.select("span.num")).replace(/\([^\)]+\)/g,"")+" "+x.select("span.before")).join("\n").replace(/<[^>]+>|전일대비 /g,""));
        },
        한강: function () {
            try{
                return type0(/<td class\="avg1"><\/td>[^>]*<\!-- 수온 -->/.exec(org.jsoup.Jsoup.connect("http://koreawqi.go.kr/wQSCHomeMainView_D.wq?action_type=T").get().select("tr.site_S01004 ").html())[0].replace(/<[^>]*>/g,"").trim());
            }catch(e){
                let bdate=new Date();
                ...
                return type0(result.select("td[nowrap]").get(2).text());
            }
        }
    };
